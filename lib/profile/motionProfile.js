...
var MotionProfile = function(type) {
	...
	this.clipboard = null; // used for cut/copy/paste
	this.copy = false;
	this.cut = false;
 ...
 * @param {Number}			segmentId	id of segment that the segment will be inserted in front of
 */
...
/**
 * Returns basic segments for the api to throw at the database
 * @return {Array}		array of basic segments
 */
...
MotionProfile.prototype.pasteSegment = function (segmentId) {
	...
	var segmentToDelete, segmentToPaste, nextSegment, nextId;
	var profile = this;
	// var profile = this;
	if (this.cut) {
		segmentToDelete = this.findById(this.clipboard);
		segmentToPaste = segmentToDelete.duplicate();
		nextSegment = this.segments.getNextSegment(segmentToDelete.id);
		nextId = nextSegment == null ? null : nextSegment.id;

		// console.log(JSON.stringify(segmentToPaste.exportData()));
		// console.log(segmentId);
		this.insertSegment(segmentToPaste, segmentId);
		this.deleteSegment(segmentToDelete.id);

		this.clipboard = segmentToPaste.id;

		this.undoManager.add({
			undo: function() {
				profile.deleteSegment(segmentToPaste.id);
				profile.insertSegment(segmentToDelete, nextId);
			},
			redo: function() {
				profile.insertSegment(newSegment, segmentId);
				profile.deleteSegment(segment.id);
			}
		});
	} else if (this.copy) {
		segment = this.findById(this.clipboard).duplicate();
		this.insertSegment(segment, segmentId);
	} else {
