...
var MotionProfile = function(type, initialConditions) {
	...
	if (this.type === "rotary") {
		Object.keys(LoadSegment.RotaryLoadsEnum).forEach(function(load) {
			...
			initLoad = LoadSegment.createLoadSegment(load, 0, Number.MAX_SAFE_INTEGER, 0, 0);
	...
	if (!initialConditions) {
		initialConditions = {
			...
			load: this.initialLoad,
			thrust: this.initialThrust,
			friction: this.initialFriction
		}
	} else {
		initialConditions = {
			position: initialConditions.position || this.initialPosition,
			velocity: initialConditions.velocity || this.initialVelocity,
			load: initialConditions.load || this.initialLoad,
			thrust: initialConditions.thrust || this.initialThrust,
			friction: initialConditions.friction || this.initialFriction
		}
	}
...
MotionProfile.prototype.setInitialConditions = function(position, velocity, load, thrust, friction) {
	...
	Object.keys(loadEnums).forEach(function(loadType, iter) {
		...
		if (profile.profileLoads[loadType].segmentData.finalTime == Number.MAX_SAFE_INTEGER) {
			switch (iter+1) {
				case 1:
...
MotionProfile.prototype.addLoadSegment = function(loadSegment, skipUndo) {
	...
	// insert or append
	if (allLoadSegs.length == 1 && allLoadSegs[0].segmentData.finalTime == Number.MAX_SAFE_INTEGER) {
...
MotionProfile.prototype.generateBasicSegments = function () {
	...
	times = _.without(times, Number.MAX_SAFE_INTEGER);
