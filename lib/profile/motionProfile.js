...
// still requires cam segments, v1 trap to a/c/a, and loads.
factory.convertV1ToV2 = function(motionProfile) {
	...
	pj.motion.points.forEach(function (point, i) {
		if (i == 0) {
			...
			// position, velocity, load, thrust, friction
			...
			if (point.segmentParameters && point.segmentParameters.segmentType === "IndexSegment") {
				...
				if (fastMath.equal(lvs[2], point.velocity)) {
					...
					console.log(lvs);
					var shape = point.segmentParameters.type.toLowerCase();
					var tf = point.time/1000;
					var tacc, tcoast;

					// var midvel = ;
					if (shape == 'trapezoidal') {
						tacc = lvs[0] + (tf-lvs[0])/3;
						tcoast = tacc + (tf-lvs[0])/3;
					} else {
						// triangular
						tacc =  lvs[0] + (tf-lvs[0])/2;
						tcoast = null;
					}

					profile.appendSegment(
						this.createAccelSegment('time-velocity', {
							t0: lvs[0],
							tf: tacc,
							p0: lvs[3],
							v0: lvs[2],
							vf: midvel,
							jPct: point.segmentParameters.jerkAccelerationPercent/100,
							mode: 'incremental'
						})
					);
					lvs = profile.getFinalValues();

					if (tcoast !== null) {
						console.log(lvs);
						console.log(tcoast);
						profile.appendSegment(
							this.createCruiseDwellSegment({
								t0: lvs[0],
								tf: tcoast,
								p0: lvs[3],
								v0: lvs[2],
								pf: lvs[2]*(tcoast-lvs[0]),
								permutation: 'time',
								mode: 'incremental'
							})
						);
					...
					profile.appendSegment(
						this.createAccelSegment('time-velocity', {
							...
							v0: lvs[2],
							pf: point.position,
							jPct: point.segmentParameters.jerkDecelerationPercent/100,
							mode: 'incremental'
						})
					);
				}
