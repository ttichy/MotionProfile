...
MotionProfile.prototype.cutSegment = function (segmentId) {
	...
	var nextSegmentId = (nextSegment == null) ? null : nextSegment.id;
	var cutSegment = this.deleteSegment(segmentId);
	...
	this.undoManager.add({
		undo: function() {
			if (!nextSegmentId) {
				...
				profile.insertSegment(cutSegment, nextSegmentId);
			}
...
MotionProfile.prototype.pasteSegment = function (segmentId) {
	if (this.clipboard !== null) {
		var pasteSegment = this.insertSegment(this.clipboard, segmentId);
		this.clipboard = pasteSegment.duplicate(); // just in case we want to paste again

		var profile = this;

		this.undoManager.add({
			undo: function() {
				profile.clipboard = profile.deleteSegment(pasteSegment.id);
			},
			redo: function() {
