...
MotionProfile.prototype.generateBasicSegments = function () {
	...
	var segTimes = _.map(this.getAllBasicSegments(), _.property('initialTime'));
	var loadTimes = _.map(this.getAllLoadSegments(), _.property('initialTime'));
	var times = _.uniq([].concat(segTimes, loadTimes, this.segments.lastSegment().finalTime)).sort();
	// console.log(JSON.stringify(segTimes));
	...
	// console.log(JSON.stringify(that.profileLoads));
	...
	// console.log(Object.keys(this.profileLoads[fri]));
	// console.log(this.profileLoads[loa].evaluateLoadAt(0));
	// console.log(JSON.stringify(this.profileLoads[fri].findSegmentWithTime(0.05)));

	var f_i, f_f, l_i, l_f, t_i, t_f;
	...
	times.forEach(function (time, i) {
		if (i < (times.length-1)) {
			...
			bsObj = {
				...
				Jerk: that.evaluateJerkAt( (times[i]+times[i+1])/2 ), // pull the jerk value from a centered times point
				// the implementation of the loads segment is unknown at that point. that must be known to correctly populate
	...
	// console.log(JSON.stringify(this.findSegmentAtTime(times[2]).segmentData));
