...
MotionProfile.prototype.cut = function (segmentId) {
...
MotionProfile.prototype.copy = function (segmentId) {
	var segment = this.findById(segmentId);
...
// right now, this function returns nothing if the paste fails. Consider throwing errors.
MotionProfile.prototype.paste = function (segmentId) {
	if (this.clipboard == null) {
		// throw new Error('There is nothing to paste!');
		return;
	}

	// figure out whether or not this function needs undomanager support.

	var segment;
	// var profile = this;
	if (this.cut) {
		segment = this.deleteSegment(this.clipboard).duplicate();
		segment.segmentData.mode = 'incremental';
