 ...
 */
MotionProfile.prototype.setInitialConditions = function(position, velocity, skipUndo) {
	...
	// 	if (profile.profileLoads[loadType].finalTime == MAX_SAFE_INTEGER) {
	// 		switch (iter+1) {
	// 			case 1:
	...
	try {
		...
		this.setInitialConditions(p, v, true);
	...
	if (!skipUndo) {
		...
		this.undoManager.add({
			undo: function () {
				that.setInitialConditions(p, v);
			},
			redo: function () {
				that.setInitialConditions(position, velocity);
			}
		});
	}
};
...
MotionProfile.prototype.getInclination = function () {
...
MotionProfile.prototype.modifySegmentValues = function(segmentId, newSegmentData, skipUndo) {
	...
	try {
		modified = segment.modifySegmentValues(newSegmentData, initialConditions);
...
MotionProfile.prototype.insertSegment = function(segment, segmentId, skipUndo) {
	...
	try {
		...
		current = this.segments.getNextSegment(newSegment.id);
		this.recalculateProfileSegments(current);
	} catch (err) {
...
MotionProfile.prototype.pasteSegment = function (segmentId) {
	if (this.clipboard !== null) {
		...
		var pastedSegment;
		try {
			pastedSegment = this.segments.insertAt(this.clipboard, segmentId);
			if (!pastedSegment) {
				this.clipboard.modifyInitialValues(originalLastValues);
				throw new Error('Pasting segment failed');
			}
		} catch (err) {
			throw new Error('Pasting segment failed: No segment selected');
		}

		var current;
		try {
			//after inserting a segment, all subsequent segments must be recalculated
			current = this.segments.getNextSegment(pastedSegment.id);
			this.recalculateProfileSegments(current);
		} catch (err) {
...
MotionProfile.prototype.addLoadSegment = function(loadSegment, skipUndo) {
	...
	if (loadSegment.segmentData.constant) {
		this.clearLoadProfile(lType);
	}

	// insert or append
	if (allLoadSegs.length == 1 && allLoadSegs[0].segmentData.constant) {
...
MotionProfile.prototype.insertLoadSegment = function (loadSegment, segmentId) {

};


MotionProfile.prototype.clearLoadProfile = function (loadType) {
	if (!loadType || !Object.keys(this.getValidLoadTypes()).includes(loadType))
		throw new Error('Clearing a load profile expects valid load type');

	this.profileLoads[loadType] = SegmentStash.makeStash();
};


MotionProfile.prototype.modifyLoadSegment = function(segmentId, newSegmentData, skipUndo) {
	if (!newSegmentData.loadType)
		throw new Error("Expecting new segment to have type");

	var lType = newSegmentData.loadType;

	//forcing new segment to be the same type as old segment
	var segment = this.profileLoads[lType].findById(segmentId);
	if (!segment)
		throw new Error("Unable to find segment with id " + segmentId + ".. is it of the same type as the old one?");

	if(this.profileLoads[lType].findOverlappingSegment(newSegmentData.initialTime, newSegmentData.finalTime))

	var oldSegmentData = {};
	Util.extend(oldSegmentData, segment.segmentData);

	if (newSegmentData.constant) {
		this.clearLoadProfile(segment.segmentData.loadType);
		this.addLoadSegment(newSegmentData);
		skipUndo = true;
	} else {
		segment.modifySegmentValues(newSegmentData);
	}
	...
	if (!skipUndo) {
		...
		this.undoManager.add({
			undo: function() {
				profile.modifyLoadSegment(segmentId, oldSegmentData);
			},
			redo: function() {
...
factory.createMotionProfile = function(type) {
	...
	// profile.appendSegment(factory.createIndexSegment({
...
factory.deserialize = function(jsonProfile) {
	...
	profileGraph.loadSegments.forEach(function(loadObj){
		var loadSeg = LoadSegment.createLoadSegment(
			loadObj.loadType,
			loadObj.initialTime,
			loadObj.finalTime,
			loadObj.initialValue,
			loadObj.finalValue
		);
