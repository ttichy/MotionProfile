...
MotionProfile.prototype.generateBasicSegments = function () {
	// get all initial times and final time from load segments and motion profiles
	var lSegments = this.getAllLoadSegments().initialTime;
	var mpSegments = [].concat(this.getAllBasicSegments().initialTime, this.finalTime);
	var times = _.sortedUniq([].concat(lSegments, mpSegments));
	...
	times.forEach(function (time, i) {
		if (i < (time.length-1)) {
			bsObj = {
				...
				InitalTime: time[i],
				FinalTime: time[i+1],
				InitialPosition: this.evaluatePositionAt(time[i]),
				FinalPosition: this.evaluatePositionAt(time[i+1]),
				InitialVelocity: this.evaluateVelocityAt(time[i]),
				FinalVelocity: this.evaluateVelocityAt(time[i+1]),
				InitialAcceleration: this.evaluateAccelerationAt(time[i]),
				FinalAcceleration: this.evaluateAccelerationAt(time[i+1]),
				Jerk: this.evaluateJerkAt( (time[i]+time[i+1])/2 ), // pull the jerk value from a centered time point
				// the implementation of the loads segment is unknown at this point. this must be known to correctly populate
				// the following values.
				InitialLoad: this.evaluateLoadAt(time[i]),
				FinalLoad: this.evaluateLoadAt(time[i+1]),
				InitialThrust: this.evaluateThrustAt(time[i]),
				FinalThrust: this.evaluateThrustAt(time[i+1]),
				InitialFriction: this.evaluateFrictionAt(time[i]),
				FinalFriction: this.evaluateFrictionAt(time[i+1])
			};
