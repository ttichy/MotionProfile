...
MotionProfile.prototype.modifySegmentValues = function(segmentId, newSegmentData) {
	...
	try {
		...
		profile.modifySegmentValues(segmentId, originalSegmentData, originalInitialConditions);
		throw new Error("Modifying the segment " + segmentId + " failed with error: " + err);
	}
...
MotionProfile.prototype.deleteSegment = function(segmentId, skipUndo) {
	...
	try {
		...
		if (!current) {
			this.appendSegment(segToDelete);
		} else {
			this.insertSegment(segToDelete, current.id);
		}
