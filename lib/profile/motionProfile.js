...
factory.convertV1ToV2 = function(motionProfileStringified) {
	var mp = JSON.parse(motionProfileStringified);
	var pj = JSON.parse(mp.ProfileJson);

	var profile = (mp.LibraryItemType === "Rotary Profile") ? this.createMotionProfile('rotary') : this.createMotionProfile('linear');
	// var that = this;

	pf.motion.points.forEach(function (point, i) {
		if (i == 1) {
			// first point is the initial conditions
			profile.setInitialConditions(point.position, point.velocity, 0, 0, 0);
		} else {
			if (point.segmentParameters.segmentType === "IndexSegment") {
				...
				if fastMath.equal(lvs[2] , point.velocity) {
					...
					var coast = this.createCoastSegment();
