...
var CruiseDwellMotionSegment = function(constructorData,loads) {
	...
	this.segmentData.finalPosition=constructorData.finalPosition;
	...
	accelSegment=AccelSegment.MakeFromTimeVelocity(this.segmentData.initialTime,
	...
	AccelSegment.AccelMotionSegment.call(this, accelSegment.getAllSegments());
...
CruiseDwellMotionSegment.prototype.modifyInitialValues = function(startPoint) {
	...
	if (this.segmentData.mode === "incremental") {
		...
		if (FastMath.lt(this.segmentData.duration, 0)) {
			throw new Error('tried to move initial time past final time for absolute cd segment for segment id: ' +this.id);
		}
		if(FastMath.lt(this.segmentData.distance,0)) {
			throw new Error("tried to move negative distance for segment id: "+this.id);
		}
	}

	// time permutation is the default
	if(this.segmentData.permutation=='distance') {
		this.segmentData.initialVelocity=v0;		
		tf=t0+this.segmentData.distance / this.segmentData.initialVelocity;
	}
