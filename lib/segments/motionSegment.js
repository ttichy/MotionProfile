...
/**
 * returns an array of all extreme values as an array of motion points
 * @return {Array} an array of motion points. One point for each extreme value.
 */
MotionSegment.prototype.getAllExtremeValues = function () {
	var allBS;
	if (typeof this !== 'basicSegment') {
		allBS = this.getAllSegments();
	} else {
		allBS = this;
	}
...
MotionSegment.prototype.getFinalValues = function() {
	...
	// if (fastMath.equal(Math.abs(vf), 0)) {
	// 	vf = 0;
	// }
