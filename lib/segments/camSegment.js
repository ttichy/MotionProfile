...
CamTable.prototype.validate = function() {
    ...
    if (master.length != this.slave.length)
    ...
    for (var i = 1; i < master.length; i++) {
        if(!Number.isFinite(master[i]))
            throw new Error("Expecting master value at row "+i+" to be finite");
        if(!Number.isFinite(master[i-1]))
            throw new Error("Expecting master value at row "+i+" to be finite");
        if(fastMath.leq(master[i],master[i-1]))
...
CamMotionSegment.prototype.validateCamData = function(camData) {
    // body...
};
