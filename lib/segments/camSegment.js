...
var CamMotionSegment = function(prevTime, prevPosition, prevVelocity,loads) {
    ...
    this.segmentData.loads=loads;

    this.segmentData.camTable=new CamTable();

    this.segmentData.camTable.master= [t0,tf];
    this.segmentData.camTable.slave=[p0,pf];
    this.segmentData.camTable.interpolation= [InterpolationEnum.CUBIC];
    this.segmentData.camTable.initialSlope= prevVelocity;
    this.segmentData.camTable.finalSlope = 0;
    ...
    var basicSegments = this.calculateBasicSegments(this.segmentData.camTable);
...
CamMotionSegment.prototype.modifyInitialValues = function(startPoint) {
    ...
    var timeDelta = initialTime-this.segmentData.camTable.master[0];
    var posDelta =initialPosition-this.segmentData.camTable.slave[0];

    for (var i = 0; i < this.segmentData.camTable.master.length; i++) {
        this.segmentData.camTable.master[i]+=timeDelta;
        this.segmentData.camTable.slave[i] +=posDelta;
    }

    this.segmentData.camTable.initialSlope=initialVelocity;

    var basicSegments = this.calculateBasicSegments(this.segmentData.camTable);

    this.initialTime=this.segmentData.camTable.master[0];
    this.finalTime=this.segmentData.camTable.master[this.segmentData.camTable.master.length-1];
...
CamMotionSegment.prototype.modifySegmentValues = function(newSegmentData) {
    ...
    if(loads) {
        this.segmentData.loads.thrust = loads.thrust;
        this.segmentData.loads.load = loads.load;
        this.segmentData.loads.friction = loads.friction;
    }

    this.segmentData.camTable.master = camTable.master;
    this.segmentData.camTable.slave = camTable.slave;
    this.segmentData.camTable.interpolation = camTable.interpolation;
    this.segmentData.camTable.finalSlope = finSlope;
...
CamMotionSegment.prototype.importFromData = function(data) {
    if(data.constructor=="CamMotionSegment") {
        var prevTime=data.camTable.master[0];
        var prevPosition=data.camTable.slave[0];
        var prevVelocity = data.camTable.initialSlope;
 ...
 * @param {number} prevVelocity previous segments' final velocity
 */
factory.Make = function(prevTime, prevPosition, prevVelocity,loads){
