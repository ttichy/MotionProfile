...
var checkCamData = function(master, slave, interpolation) {
    ...
    if (interpolation.length != master.length - 1)
...
var CamMotionSegment = function(prevTime, prevPosition, prevVelocity,loads) {
    ...
    this.segmentData= {
        master: [t0,tf], 
        slave: [p0,pf], 
        interpolation: [InterpolationEnum.CUBIC], 
        initialSlope: prevVelocity, 
        finalSlope:0};
    ...
    var basicSegments = this.calculateBasicSegments(this.segmentData.master,
     this.segmentData.slave, this.segmentData.interpolation, this.segmentData.initialSlope, this.segmentData.finalSlope);
 ...
 */
CamMotionSegment.prototype.modifyInitialValues = function(initialTime, initialPosition, initialVelocity) {
    if(initialTime < 0)
 ...
 */
CamMotionSegment.prototype.modifySegmentValues = function(master, slave, interpolation,finalSlope,loads) {
    checkCamData(master,slave,interpolation);

    var finSlope = finalSlope || 0;
    if(loads)
    {
    ...
    this.segmentData.master=master;
    this.segmentData.slave=slave;
    this.segmentData.interpolation=interpolation;
 ...
 */
CamMotionSegment.prototype.calculateBasicSegments = function(master, slave, interpolation, initialSlope, finalSlope) {
